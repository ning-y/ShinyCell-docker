...
wrSVfix <- function() {
  glue::glue('### Useful stuff \n',
			 ...
			 '			   c("#4575B4", "#74ADD1", "#ABD9E9", "#E0F3F8", "#FFFFFF", \n ',
			 '				 "#F08080", "#CD5C5C", "#B22222", "#8B0000")[c(1,1:9,9)], \n ',
             '             c("#FDE725","#AADC32","#5DC863","#27AD81","#21908C", \n',
             '               "#2C728E","#3B528B","#472D7B","#440154")) \n',
             'names(cList) = c("White-Red", "Blue-Yellow-Red", "Blue-White-Red", "Yellow-Green-Purple") \n',
             ' \n',
             '# Panel sizes \n',
			...
			'   if (inpPlt != "HeatmapByCell") {{  # Added by DTH  \n',
			...
			'   }} \n',
			'   else  # HeatmapByCell \n ',
			'	{{  ggOut = ggplot(ggData, aes(x = sampleID, y = geneName, fill = val)) + \n ',
			'		geom_tile() + sctheme(base_size = sList[inpfsz], Xang = 45, XjusH = 1) + \n ',
			'		facet_wrap(~ grpBy, scales = "free_x") + \n ',
			'		scale_fill_gradientn(inpScl, limits = colRange, colours = cList[[inpcols]]) + \n ',
			'		theme_minimal() + guides(fill = guide_colorbar(direction= "horizontal", barwidth = 15)) + \n',
			...
			'       grid.arrange(ggOut, ggLeg, #ggRow, # Dave commented this out when implementing heatmapbycell - probably need to resolve ggRow in this situation \n',
...
wrSVmain <- function(prefix, subst = "") {
  glue::glue(#'optCrt="{{ option_create: function(data,escape) {{return(\'<div class=\\"create\\"><strong>\' + \'</strong></div>\');}} }}" \n', #PY moved to wrSVfix
             ...
             # '   \n',             
			#copied #75 part 2
			'    if(input${prefix}d1plt %in% c("Bubbleplot", "Heatmap", "HeatmapByCell")){{ \n',
             '        scBubbHeat({prefix}conf, {prefix}meta, input${prefix}d1inp, input${prefix}d1grp, input${prefix}d1plt,  \n',
             '             input${prefix}d1sub1, input${prefix}d1sub2, "{prefix}gexpr.h5", {prefix}gene,  \n',
             '             input${prefix}d1scl, input${prefix}d1row, input${prefix}d1col,  \n',
             '             input${prefix}d1cols, input${prefix}d1fsz)  \n',
             ...
             '    content = function(file) {{ ggsave(  \n',
             '      file, device = "pdf", height = input${prefix}d1oup.h, width = input${prefix}d1oup.w,  \n',
             '      plot =  if(input${prefix}d1plt %in% c("Bubbleplot", "Heatmap", "HeatmapByCell")){{ \n',
             '                  scBubbHeat({prefix}conf, {prefix}meta, input${prefix}d1inp, input${prefix}d1grp, input${prefix}d1plt,  \n',
             '                       input${prefix}d1sub1, input${prefix}d1sub2, "{prefix}gexpr.h5", {prefix}gene,  \n',
             '                       input${prefix}d1scl, input${prefix}d1row, input${prefix}d1col,  \n',
             '                       input${prefix}d1cols, input${prefix}d1fsz)  \n',
...
wrUImain <- function(prefix, subst = "", ptsiz = "1.25") {
  glue::glue('  ### Tab1.a1: cellInfo vs geneExpr on dimRed \n',
             ...
             '                           choices = c("White-Red","Blue-Yellow-Red","Blue-White-Red","Yellow-Green-Purple"), \n',
             ...
             '                           choices = c("White-Red", "Blue-Yellow-Red", "Blue-White-Red",\n',
             '                                       "Yellow-Green-Purple"), \n',
             '                           selected = "Blue-Yellow-Red"), \n',
             ...
             '        radioButtons("{prefix}d1plt", "Plot type:",  \n',
             '                     choices = c("Bubbleplot", "Heatmap", "HeatmapByCell", "Stacked Violin", "Stacked Boxplot"),  \n',
